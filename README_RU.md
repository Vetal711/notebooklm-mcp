...
**Ошибка: `Authentication expired`**
Если вы получаете такую ошибку в Claude/Cursor, это означает, что у Playwright-сессии истек срок действия токенов Google. 
**ВНИМАНИЕ: НИКОГДА не пытайтесь запускать логин прямо из чата ИИ-агента!** Агенты работают в фоновой сессии Windows, из-за чего браузер открывается невидимым, и процесс навсегда зависает.

**Решение:** 
1. Откройте настоящий терминал ОС (или просто кликните дважды по скрипту).
2. Запустите `login.bat` (для Windows) или `login.sh` (для Mac/Linux) из корневой папки проекта.
3. Появится окно браузера. Авторизуйтесь, и окно само закроется.
4. Убедитесь, что в файле конфигурации вашего MCP-клиента (например, `claude_desktop_config.json`) прописан параметр `"cwd": "/absolute/path/to/notebooklm-mcp"`. Это гарантирует, что сервер всегда запускается из правильной папки и не теряет сессию.
